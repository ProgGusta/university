...
int popStack();
int readStack();
void startArv(char *arvore);
void putArv(int p);
void penta2Bi(char *arvoreNew);
void PostOrder(char *arvore, int indice);
void Polo2Pcode();
void printArv(char *arvore, int indice, int nivel);
...
void printArv(char *arvore, int indice, int nivel) 
{
    if (indice < MAX_SIZE && arvore[indice] != ' ') 
    {
        ...
        printArv(arvore, 5 * indice + 1, nivel + 1); 
        printArv(arvore, 5 * indice + 2, nivel + 1); 
        printArv(arvore, 5 * indice + 3, nivel + 1); 
        printArv(arvore, 5 * indice + 4, nivel + 1); 
        printArv(arvore, 5 * indice + 5, nivel + 1); 
    }
}
