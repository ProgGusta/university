...
/* Variáveis */
char labels[TAMMAX][TAMMAX][5]; // Lista de produção
int tpLabel[TAMMAX]; // Topo de labels
...
char* traduz(int codigo); // melhor visualização da arvore
void printArv(int *arvore, int indice, int nivel); // Print Arv
...
conteúdo */ 
    if ((in_fp = fopen("casos.txt", "r")) == NULL) 
        printf("ERROR - cannot open casos.txt \n"); 
    else 
    {
        getChar();
        ...
        do{
            ...
            printArv(arvore[tpMat], 0, 0);

            printf("\n\nLista de producoes: \n");
            for(int i = 0; i < tpLabel[tpMat]; i++){
                printf(" %s ", labels[tpMat][i]);
            }
...
void printArv(int *arvore, int indice, int nivel) {
    if (indice < TAMMAX && arvore[indice] != 0) {
        ...
        printArv(arvore,3 * indice + 1, nivel + 1); 
        printArv(arvore, 3 * indice + 2, nivel + 1); 
        printArv(arvore, 3 * indice + 3, nivel + 1); 
    }
}
...
{ 
    ...
    { 
        if(nextToken == ADD_OP)
        {
            ...
            strcpy(labels[tpMat][ind], "p2");
        }
        else
        {
            arvore[tpMat][3*nvl + 2] = SUB_OP;
            strcpy(labels[tpMat][ind], "p3");
        }
        lex();
...
{ 
    ...
    { 
        if(nextToken == MULT_OP)
        {
            ...
            strcpy(labels[tpMat][ind], "p5");
        }
        else
        {
            arvore[tpMat][3*nvl + 2] = DIV_OP;
            strcpy(labels[tpMat][ind], "p6");
        }
        lex(); 
...
void factor(int nvl) 
{
    ...
    if (nextToken == IDENT || nextToken == INT_LIT)
    {
        ...
        if(nextToken == IDENT)
        {
            ...
            strcpy(labels[tpMat][tpLabel[tpMat]], "p7");
            tpLabel[tpMat]++;
        }
        else
        {
            arvore[tpMat][3 * nvl + 1] = INT_LIT;
            strcpy(labels[tpMat][tpLabel[tpMat]], "p8");
            tpLabel[tpMat]++;
        }
        lex();
    }
    ...
    { 
        ...
        { 
            ...
            strcpy(labels[tpMat][tpLabel[tpMat]], "p9");
            tpLabel[tpMat]++; 
